...
describe('Saras axios instance', () => {
  ...
  beforeEach(() => {
    axiosInstance = new SarasAxios('mock-token-value', mockLogger as any as Logger) as unknown as MockSarasAxios;
  ...
  describe('pre-request interceptor', () => {
    ...
    test('injects content-type into request', async () => {
      const mockRequest = { headers: { 'Content-Type': 'application/json' } };

      const req = await axiosInstance.instance.interceptors.request.handlers[0].fulfilled(mockRequest);

      expect(req).toHaveProperty('headers.Content-Type', 'application/json');
    });
  });
