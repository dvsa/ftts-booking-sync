...
describe('Booking synchroniser - error handling', () => {
  ...
  describe('processBookings', () => {
    describe('new bookings', () => {
      const mockBookings = [
        { bookingProduct: { id: '001' } },
        { bookingProduct: { id: '002' } },
      ];
      ...
      describe('if one of the CRM updates fails', () => {
        describe('with a CrmError with status code 400 or 500', () => {
          test.each([
            ...
            expect(mockSARAS.deleteBooking).toHaveBeenCalledWith(mockBookingRefs[0]);
            expect(mockSARAS.deleteBooking).toHaveBeenCalledWith(mockBookingRefs[1]); // Continues to second booking
          });
        });
